...
### Bulk load from Excel (`xlsx_fields.go` + `xlsx_template.go` + `xlsx_import.go`)

`GET /import/template.xlsx` generates the capture template and `POST /import/xlsx` reads it back (separate from the older JSON `POST /import`, which stays). All three files derive from **one** column table in `xlsx_fields.go` (`activityColumns()` / `deliverableColumns()`): header, kind, required flag, dropdown source, question text and glossary help. Adding a question means adding one `xlsxColumn` plus its case in `parseActivityRow` — never a second parallel list. Column headers deliberately match the ones `/export/xlsx` emits, and `codedField.resolve` accepts both the Spanish dropdown label and the raw model key (accent- and case-insensitive), so an exported workbook can be edited and re-uploaded.

The capture sheets are laid out as a form, not a bare grid: row 1 is a merged colored band per questionnaire block (`writeGroupBand`, colors from `svg.SectionPalette` via `groupColor`), row 2 holds the headers, data starts at row 3 (`bandRow`/`headerRow`/`firstDataRow`). Because of that the importer does **not** assume headers are in row 1 — `findHeaderRow` scans the first `headerScanRows` rows and picks the best match, which is also what keeps `/export/xlsx` files (headers in row 1) importable. Scale questions are dropdowns carrying their anchor text ("4 — Apoya un objetivo estratégico principal"), so `parseScaleCell` takes the leading digits and still accepts a bare number. Each column gets **exactly one** data validation (two on the same range fight each other in Excel): `dynamic` (live ranges like the activity-name dropdown on the Entregables sheet, built by `deliverableNameLists`) wins over the fixed `Listas` ranges, which win over a prompt-only validation. Anything to the right of the last questionnaire column is a `helperColumn` — computed with formulas (`¿Fila completa?`, `Duración (días)`) and ignored by the import precisely because mapping is by header name.

The import is **all-or-nothing**: `parseImportWorkbook` is pure (it never touches the store) and returns either activities to create or the full list of `hoja/fila/columna` errors; the handler only calls `CreateActivity` when `Errors` is empty. Rows whose first cell starts with `EJEMPLO` are the template's own sample rows and are skipped, so an untouched template is safe to upload. Deliverables live on their own sheet and are matched to activities by name (hence duplicate activity names are a hard error), with `DLV-NNN` IDs assigned in sheet order and the "Depende de" column resolved from name to ID at import time.

The `Gantt` sheet is a native stacked-bar chart, not a picture: `writeGanttSheet` writes formulas that read the `Entregables` sheet (offset and duration in days from the portfolio start) and the first series is filled white over a white chart/plot area so only the duration bar shows (plain "no fill" is what you'd expect, but LibreOffice tints it anyway). Details worth preserving: the workbook sets `FullCalcOnLoad` because excelize writes formulas without cached values; example dates are written as `time.Time` (real Excel serials) since ISO strings break the date arithmetic; series names are given as **cell references** to the table headers, because a literal name travels as an invalid formula and the legend ends up reading "Column F"; and formulas stay simple — `"…"&count&"…"` inside a nested `IF` breaks LibreOffice/excelize's parser, which is why `rowStatusFormula` says "Faltan datos" instead of counting them. In excelize's bar chart, `XAxis` is the category axis (tasks, reversed so the first is on top) and `YAxis` is the value axis (days); they read backwards from the visual orientation.
